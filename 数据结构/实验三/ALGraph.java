...
public class ALGraph<AnyType extends Comparable<?super AnyType>> {
	...
	public void createUDN() {
		...
		for (int v = 0; v < vexNum; v++)// 构造顶点向量
			vexs[v] = new VNode<AnyType>((AnyType)in.next());
		System.out.println("请输入各边顶点及其权值");
		for (int k = 0; k < arcNum; k++) {
			int v = locateVex((AnyType)in.next());
	...
	// 图的邻接表存储结构表示中的顶点节点类
	public static class VNode<AnyType extends Comparable<?super AnyType>> {
	...
	// 图中邻接表存储结构表示中的边节点类
	public static class ArcNode<AnyType extends Comparable<?super AnyType>> {
	...
	public static void main(String[] y) throws Exception {
		...
		GraphTraverse.BFSTraverse(alGraph);
		System.out.println();
		GraphTraverse.DFSTraverse(alGraph);
