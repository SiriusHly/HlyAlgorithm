...
public class StudentSituation extends JPanel implements ActionListener {
	...
	JTextField 学号, 姓名, 所学专业, 家庭住址, 联系电话;
	JRadioButton 男, 女;
	Student 学生 = null;
	...
	public void actionPerformed(ActionEvent e) { if(e.getSource()==录入) { String number="";
	number=学号.getText(); if(number.length()>0) { try { inOne=new FileInputStream(file); 
	inTwo=new ObjectInputStream(inOne); 基本信息表=(Hashtable)inTwo.readObject(); inOne.close(); inTwo.close();
	} catch(Exception ee) { } if(基本信息表.containsKey(number)) { String warning="该生基本信息已存在,请到修改页面修改!"; 
...
姓名.setText(null); 所学专业.setText(null);
